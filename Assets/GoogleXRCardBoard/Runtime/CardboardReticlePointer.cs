...
public class CardboardReticlePointer : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, _RETICLE_MAX_DISTANCE))
        {
            ...
            IPointerEvents pointerEvents = hitObject.GetComponent<IPointerEvents>();

            if (_gazedAtObject != hitObject)
            {
                if (_gazedAtObject != null)
                {
                    var previousPointerEvents = _gazedAtObject.GetComponent<IPointerEvents>();
                    previousPointerEvents?.OnPointerExit();
                }

                _gazedAtObject = hitObject;
                pointerEvents?.OnPointerEnter();
            }
        ...
        else
        {
            if (_gazedAtObject != null)
            {
                var pointerEvents = _gazedAtObject.GetComponent<IPointerEvents>();
                pointerEvents?.OnPointerExit();
            }
            _gazedAtObject = null;
            ResetParams();
        }

        if (Google.XR.Cardboard.Api.IsTriggerPressed && _gazedAtObject != null)
        {
            var pointerEvents = _gazedAtObject.GetComponent<IPointerEvents>();

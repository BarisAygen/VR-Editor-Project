...
public class SelectTransformGizmo : MonoBehaviour
{
    ...
    private int runtimeTransformLayer = 7;
    ...
    void Update()
    {
        ...
        if (!EventSystem.current.IsPointerOverGameObject() && Physics.Raycast(ray, out raycastHit)) //Make sure you have EventSystem in the hierarchy before using EventSystem
        {
            ...
            if (highlight.CompareTag("GridCell") && highlight != selection)
            {
            ...
            if (Physics.Raycast(ray, out raycastHit))
            {
                ...
                else
                {
                    if (selection)
                    {
                        ...
                        SyncScript.UpdateFromFirebase(selection);
        ...
        if (runtimeTransformGameObj.activeSelf)
        {
            ...
            if (Input.GetKeyDown(KeyCode.V))
            {
                if (selection)
                {
                    ...
                    // Custom function calls
                    SyncScript.UpdateFromFirebase(selection);
                    SyncScript.EditObject(selection);
                    selection = null;
                    runtimeTransformGameObj.SetActive(false);
            ...
            if (Input.GetKeyDown(KeyCode.Delete))
            {
                if (selection)
                {
                    // Check if the selection has any children
                    if (selection.childCount > 0)
                    {
                        // Detach all children
                        while (selection.childCount > 0)
                        {
                            Transform child = selection.GetChild(0);
                            child.SetParent(null);
                        }
                    }

                    // Now handle the deletion of the parent object
                    selection.GetComponent<MeshRenderer>().material = originalMaterialSelection;
                    SyncScript.RemoveObject(selection); // Assuming this function deletes the GameObject in Unity and handles Firebase updates
                    selection = null;
                    runtimeTransformGameObj.SetActive(false);
                }
            }
